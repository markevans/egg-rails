module Egg
  module ViewHelpers
    ...
    def egg_mustache_templates
      Dir[Rails.root.join('app/assets/javascripts/templates/*.mustache')].map do |path|
        egg_mustache_template(File.basename(path, '.mustache'))
      end.join("\n").html_safe
    end

    def egg_mustache_template(path)
      content = Rails.root.join('app/assets/javascripts/templates', "#{path}.mustache").read.html_safe
